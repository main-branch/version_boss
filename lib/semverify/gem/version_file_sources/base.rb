  ...
  module Gem
    module VersionFileSources
      ...
      class Base
        ...
        #
        # @example
        #   version_file = Semverify::Gem::VersionFileSources::Gemspec.find
        #
        # @return [Semverify::Gem::VersionFile, nil] the version file or nil if no version file was found
        #
        def self.find
          Dir[glob].filter_map do |path|
            if (match = File.read(path).match(content_regexp))
              version = Semverify::Gem::IncrementableVersion.new(match[:version])
              Semverify::Gem::VersionFile.new(path, match[:content_before], version, match[:content_after])
            end
          end.first
        end
